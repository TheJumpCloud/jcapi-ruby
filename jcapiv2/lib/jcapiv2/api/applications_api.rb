...
module JCAPIv2
  class ApplicationsApi
    ...
    # @param application_id ObjectID of the Application.
    # @param [Hash] opts the optional parameters
    # @option opts [ImportUsersRequest] :body 
    # @option opts [String] :x_org_id Organization identifier that can be obtained from console settings.
    # @return [JobId]
    def import_create(application_id, opts = {})
      data, _status_code, _headers = import_create_with_http_info(application_id, opts)
      data
    end
    ...
    def import_create_with_http_info(application_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: ApplicationsApi.import_create ...'
      end
      # verify the required parameter 'application_id' is set
      if @api_client.config.client_side_validation && application_id.nil?
        fail ArgumentError, "Missing the required parameter 'application_id' when calling ApplicationsApi.import_create"
      end
      # resource path
      local_var_path = '/applications/{application_id}/import/jobs'.sub('{' + 'application_id' + '}', application_id.to_s)
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: ApplicationsApi#import_create\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
