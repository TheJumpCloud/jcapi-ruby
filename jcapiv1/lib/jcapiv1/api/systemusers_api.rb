...
module JCAPIv1
  class SystemusersApi
    ...
    def systemusers_totp_info(id, opts = {})
      data, _status_code, _headers = systemusers_totp_info_with_http_info(id, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [String] :x_org_id 
    # @return [Array<(Totpenrollmentinfo, Integer, Hash)>] Totpenrollmentinfo data, response status code and response headers
    def systemusers_totp_info_with_http_info(id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SystemusersApi.systemusers_totp_info ...'
      end
      # verify the required parameter 'id' is set
      if @api_client.config.client_side_validation && id.nil?
        fail ArgumentError, "Missing the required parameter 'id' when calling SystemusersApi.systemusers_totp_info"
      end
      # resource path
      local_var_path = '/systemusers/{id}/totpinfo'.sub('{' + 'id' + '}', id.to_s)
      ...
      return_type = opts[:return_type] || 'Totpenrollmentinfo' 
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SystemusersApi#systemusers_totp_info\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
