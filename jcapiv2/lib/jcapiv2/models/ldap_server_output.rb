  ...
  class LdapServerOutput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv2::LdapServerOutput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv2::LdapServerOutput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      user_lockout_action_validator = EnumAttributeValidator.new('', ['disable', 'remove'])
      return false unless user_lockout_action_validator.valid?(@user_lockout_action)
      user_password_expiration_action_validator = EnumAttributeValidator.new('', ['disable', 'remove'])
    ...
    def user_lockout_action=(user_lockout_action)
      validator = EnumAttributeValidator.new('', ['disable', 'remove'])
