  ...
  class Systemuserput
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv1::Systemuserput` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv1::Systemuserput`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [account_locked, addresses, allow_public_key, alternate_email, attributes, company, cost_center, department, description, disable_device_max_login_attempts, displayname, email, employee_identifier, employee_type, enable_managed_uid, enable_user_portal_multifactor, external_dn, external_password_expiration_date, external_source_type, externally_managed, firstname, job_title, lastname, ldap_binding_user, location, managed_apple_id, manager, mfa, middlename, password, password_never_expires, phone_numbers, public_key, relationships, samba_service_user, ssh_keys, state, sudo, suspended, tags, unix_guid, unix_uid, username].hash
    end
