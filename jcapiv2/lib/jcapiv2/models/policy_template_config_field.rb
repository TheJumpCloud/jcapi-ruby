  ...
  class PolicyTemplateConfigField
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv2::PolicyTemplateConfigField` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv2::PolicyTemplateConfigField`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'validators')
        self.validators = attributes[:'validators']
      end
    end
    ...
    def valid?
      display_type_validator = EnumAttributeValidator.new('Object', ['checkbox', 'date', 'email', 'file', 'number', 'select', 'text', 'textarea', 'singlelistbox', 'doublelistbox', 'table', 'segmentedbutton', 'radio', 'copywell', 'timeinput', 'datepickerrange', 'multilist'])
    ...
    def display_type=(display_type)
      validator = EnumAttributeValidator.new('Object', ['checkbox', 'date', 'email', 'file', 'number', 'select', 'text', 'textarea', 'singlelistbox', 'doublelistbox', 'table', 'segmentedbutton', 'radio', 'copywell', 'timeinput', 'datepickerrange', 'multilist'])
    ...
    def hash
      [default_value, display_options, display_type, id, label, name, position, read_only, required, sensitive, tooltip, validators].hash
    end
