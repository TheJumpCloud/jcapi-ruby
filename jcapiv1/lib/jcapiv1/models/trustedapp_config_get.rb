  ...
  class TrustedappConfigGet
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv1::TrustedappConfigGet` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv1::TrustedappConfigGet`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'trusted_apps')
        if (value = attributes[:'trusted_apps']).is_a?(Array)
          self.trusted_apps = value
        end
      end
    end
    ...
    def list_invalid_properties
      ...
      if @trusted_apps.nil?
        invalid_properties.push('invalid value for "trusted_apps", trusted_apps cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @trusted_apps.nil?
      true
    end
