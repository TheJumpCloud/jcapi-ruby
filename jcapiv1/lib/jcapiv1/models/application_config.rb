  ...
  class ApplicationConfig
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv1::ApplicationConfig` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv1::ApplicationConfig`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'constant_attributes')
        self.constant_attributes = attributes[:'constant_attributes']
      end

      if attributes.key?(:'database_attributes')
        self.database_attributes = attributes[:'database_attributes']
      end
      ...
      if attributes.key?(:'idp_private_key')
        self.idp_private_key = attributes[:'idp_private_key']
      end
    ...
    def hash
      [acs_url, constant_attributes, database_attributes, idp_certificate, idp_entity_id, idp_private_key, sign_assertion, sign_response, sp_entity_id].hash
    end
