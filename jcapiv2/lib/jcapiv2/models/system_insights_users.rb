  ...
  class SystemInsightsUsers
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv2::SystemInsightsUsers` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv2::SystemInsightsUsers`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'ad_managed')
        self.ad_managed = attributes[:'ad_managed']
      end
      ...
      if attributes.key?(:'real_user')
        self.real_user = attributes[:'real_user']
      end
    ...
    def hash
      [ad_managed, admin, collection_time, description, directory, gid, gid_signed, last_login, managed, real_user, shell, suspended, system_id, type, uid, uid_signed, username, uuid].hash
    end
