...
module JCAPIv1
  class ApplicationtemplateOidc
    # The grant types allowed.
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv1::ApplicationtemplateOidc` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv1::ApplicationtemplateOidc`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      token_endpoint_auth_method_validator = EnumAttributeValidator.new('Object', ['client_secret_basic', 'client_secret_post', 'none'])
    ...
    def token_endpoint_auth_method=(token_endpoint_auth_method)
      validator = EnumAttributeValidator.new('Object', ['client_secret_basic', 'client_secret_post', 'none'])
    ...
    def hash
      [grant_types, redirect_uris, sso_url, token_endpoint_auth_method].hash
    end
