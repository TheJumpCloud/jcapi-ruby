...
module JCAPIv2
  class SystemGroupAssociationsApi
    ...
    # @option opts [String] :details This will provide detail descriptive response for the request.
    # @return [Array<CommandsGraphObjectWithPaths>]
    ...
    # @option opts [Array<String>] :filter A filter to apply to the query.  **Filter structure**: &#x60;&lt;field&gt;:&lt;operator&gt;:&lt;value&gt;&#x60;.  **field** &#x3D; Populate with a valid field from an endpoint response.  **operator** &#x3D;  Supported operators are: eq, ne, gt, ge, lt, le, between, search, in. _Note: v1 operators differ from v2 operators._  **value** &#x3D; Populate with the value you want to search for. Is case sensitive. Supports wild cards.  **EX:** &#x60;GET /api/v2/groups?filter&#x3D;name:eq:Test+Group&#x60;
    # @option opts [String] :details This will provide detail descriptive response for the request.
    # @return [Array<(Array<CommandsGraphObjectWithPaths>, Integer, Hash)>] Array<CommandsGraphObjectWithPaths> data, response status code and response headers
    def graph_system_group_traverse_command_with_http_info(group_id, opts = {})
      ...
      if @api_client.config.client_side_validation && opts[:'details'] && !['v1'].include?(opts[:'details'])
        fail ArgumentError, 'invalid value for "details", must be one of v1'
      end
      # resource path
      ...
      return_type = opts[:return_type] || 'Array<CommandsGraphObjectWithPaths>' 
    ...
    def graph_system_group_traverse_policy_group_with_http_info(group_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: SystemGroupAssociationsApi.graph_system_group_traverse_policy_group ...'
      end
      # verify the required parameter 'group_id' is set
      if @api_client.config.client_side_validation && group_id.nil?
        fail ArgumentError, "Missing the required parameter 'group_id' when calling SystemGroupAssociationsApi.graph_system_group_traverse_policy_group"
      end
      # resource path
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: SystemGroupAssociationsApi#graph_system_group_traverse_policy_group\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
