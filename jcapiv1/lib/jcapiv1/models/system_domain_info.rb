  ...
  class SystemDomainInfo
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `JCAPIv1::SystemDomainInfo` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `JCAPIv1::SystemDomainInfo`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'part_of_domain')
        self.part_of_domain = attributes[:'part_of_domain']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          ...
          part_of_domain == o.part_of_domain
    end
    ...
    def hash
      [domain_name, part_of_domain].hash
    end
